...
func (gs *Service) Serve() {
	// 公会服务不需要持续运行的协程
}

// CreateGuild 创建公会
func (gs *Service) CreateGuild(leaderId int64, leaderName string, guildName string) (*Guild, error) {
	// 生成公会ID (这里简化处理，实际应该从数据库获取或使用更复杂的生成逻辑)
	guildId := int64(0) // 应该从数据库或全局ID生成器获取
...
// JoinGuild 加入公会
func (gs *Service) JoinGuild(guildId int64, playerId int64, playerName string) error {
	return gs.joinGuildInternal(playerId, playerName, guildId)
}

// LeaveGuild 离开公会
func (gs *Service) LeaveGuild(playerId int64) error {
	// 检查玩家是否已加入公会
...
// DisbandGuild 解散公会
func (gs *Service) DisbandGuild(guildId int64) error {
	// 获取公会
	guildInterface, exists := gs.guilds.Get(guildId)
	if !exists {
		return nil // 公会不存在
...
// GetGuild 获取公会信息
func (gs *Service) GetGuild(guildId int64) (*Guild, bool) {
...
func (gs *Service) GetGuildByPlayer(playerId int64) (*Guild, bool) {
	guildIdInterface, exists := gs.playerGuild.Get(playerId)
	if !exists {
		return nil, false
	}
...
func (gs *Service) GetPlayerGuildInfo(playerId int64) (*Guild, interface{}, bool) {
	guild, exists := gs.GetGuildByPlayer(playerId)
	if !exists {
	...
	guildMembers, err := gs.GetGuildMembers(guild.GuildId)
	if err != nil {
		return guild, nil, true
	}
...
func (gs *Service) GetGuildMembers(guildId int64) ([]*GuildMember, error) {
	// 检查公会是否存在
	guildInterface, exists := gs.guilds.Get(guildId)
	if !exists {
...
func (gs *Service) transferGuildLeader(guild *Guild, oldLeaderId int64) {
	// 寻找新的会长（副会长 -> 精英 -> 普通成员）
...
func (gs *Service) CheckGuildPermission(playerId int64, permission int64) error {
	// 检查玩家是否已加入公会
	guildIdInterface, exists := gs.playerGuild.Get(playerId)
	if !exists {
...
// ApplyGuild 申请加入公会
func (gs *Service) ApplyGuild(applyId int64, playerId int64, playerName string, guildId int64, remark string) error {
	// 检查玩家是否已加入其他公会
...
func (gs *Service) SetGuildMemberPosition(operatorId int64, targetPlayerId int64, newPosition int) error {
	// 检查操作者是否有权限
...
func (gs *Service) KickGuildMember(operatorId int64, targetPlayerId int64, reason string) error {
	// 检查操作者是否有权限
...
func (gs *Service) UpdateGuildNotice(operatorId int64, notice string) error {
	// 检查操作者是否有权限
...
func (gs *Service) ProcessGuildApply(operatorId int64, applyId int64, accept bool, remark string) error {
	// 检查操作者是否有权限
	...
	if accept {
		...
		// 接受申请，加入公会
		gs.joinGuildInternal(apply.PlayerId, apply.PlayerName, apply.GuildId)
...
func (gs *Service) UpdateGuildMemberOnlineStatus(playerId int64, online bool) error {
	// 检查玩家是否已加入公会
	guildIdInterface, exists := gs.playerGuild.Get(playerId)
	if !exists {
		return nil // 玩家未加入公会
...
func (gs *Service) UpdateGuildMemberContribution(playerId int64, amount int64) error {
	// 检查玩家是否已加入公会
	guildIdInterface, exists := gs.playerGuild.Get(playerId)
	if !exists {
		return ErrPlayerNotInGuild
...
func (gs *Service) GetGuildMemberContribution(playerId int64) (int64, error) {
	// 检查玩家是否已加入公会
	guildIdInterface, exists := gs.playerGuild.Get(playerId)
	if !exists {
...
// UpgradeGuild 升级公会
func (gs *Service) UpgradeGuild(operatorId int64) error {
...
// joinGuildInternal 加入公会（内部实现）
func (gs *Service) joinGuildInternal(playerId int64, playerName string, guildId int64) error {
	// 检查公会是否存在
	guildInterface, exists := gs.guilds.Get(guildId)
	if !exists {
		return nil // 公会不存在
